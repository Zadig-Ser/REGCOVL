...
const generatedControllers = {
  ...
  init: router => {
    ...
    router.get(baseUrl + "/findBy_department/:key", authorize([]), CourseController.findBy_department);
  ...
  */
  ...
  /**
  * courseModel.findBy_department
  *   @description CRUD ACTION findBy_department
  *   @param Objectid key Id of model to search for
  *
  */
  findBy_department: async (req, res) => {
    try {
      const result = await CourseModel.findBy_department(req.params.key);
      res.json(result);
    } catch (err) {
      const safeErr = ErrorManager.getSafeError(err);
      res.status(safeErr.status).json(safeErr);
